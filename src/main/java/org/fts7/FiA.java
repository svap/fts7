...
/**
 * Формиование списка позиций слова
...
for(int pn:pp)
              {Fi a=new Fi(); a.idw=idw; a.p=pn; add(a); }
}

/**
 *                                  OLD Version, compatible with old indx format!
 * @param idw идентификатор слова
 * @param p   массив позиций слова
void addFiA(int idw, byte[] p) {
Fi a;
int n;

//n=b2ToInt(p[0],p[1]); // старший байт
if ((p[1] & 0x80) !=0) // длина элемента 2 байта
                      for (int i=2; i<p.length-1; i+=2)
                           { a=new Fi(); a.idw=idw;
                             a.p= b2ToInt(p[i],p[i+1]);
                             add(a);
                           }
else // длина элемента 1 байт
     for (int i=2; i<p.length; i++)
     { a=new Fi(); a.idw=idw; a.p=p[i]&0xff; add(a);}
}    
*/
...
double d;
                             //======================================
for (int i=x=0; i<lsize; i++){ // поиск всех возможных цепочек
