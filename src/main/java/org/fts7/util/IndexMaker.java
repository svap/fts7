...
/**
 * This utility builts search index file from an index profile. The index
 * profile is an XML file with .ip extension, contains all settings to build
 * search index. You can found the example of profile in the Index profiles
 * subdirectory of the fts7.zip file.
 * <p>
...
// Indexing paths
        try
        {
            ...
            FileIndexer si = new FileIndexer(indexFile, create);
            for (PathItem p : this)
                if (p.enable)
                {
                    System.out.printf("\nAdding path: %s\n", p.path);
